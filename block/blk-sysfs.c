				     ...
				     size_t count)
{
	...
	/* enable dm wbt(blk mode) for 950 O product */
	#ifdef CONFIG_WBT_DM_BLK
...
#ifdef CONFIG_DIRTY_PAGES_CONTROL
static ssize_t queue_dirty_pages_controlled_show(struct request_queue *q,
								char *page)
{
	return sprintf(page, "%d\n", dirty_pages_controlled ? 1 : 0);
}

static ssize_t queue_dirty_pages_controlled_store(struct request_queue *q,
					const char *page, size_t count)
{
	ssize_t ret;
        unsigned long val;
	...
	if (ret < 0 || val > 1)
	    return -EINVAL;
	dirty_pages_controlled = (val == 1 ? true : false);
...
#ifdef CONFIG_DIRTY_PAGES_CONTROL
static struct queue_sysfs_entry queue_dirty_pages_controlled_entry = {
        .attr = {.name = "dirty_pages_controlled", .mode = S_IRUGO | S_IWUSR },
        .show = queue_dirty_pages_controlled_show,
        .store = queue_dirty_pages_controlled_store,
};
#endif
...
#ifdef CONFIG_DIRTY_PAGES_CONTROL
	&queue_dirty_pages_controlled_entry.attr,
#endif
	NULL,
};
