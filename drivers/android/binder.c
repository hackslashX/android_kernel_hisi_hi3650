...
retry:
	...
	while (1) {
		...
		if (put_user(cmd, (uint32_t __user *)ptr)) {
			...
			pr_debug("Failed to call put_user, point:%p, ptr:%p, buffer:%p, size:%zu, *consumed:%zu, cmd:%u\n",
		...
		if (copy_to_user(ptr, &tr, sizeof(tr))) {
			...
			pr_debug("Failed to call copy_to_user, point:%p, ptr:%p, buffer:%p, size:%zu, *consumed:%zu, cmd:%u\n",
				t, ptr, buffer, size, *consumed, cmd);
			return -EFAULT;
		}
		ptr += sizeof(tr);
...
err_bad_arg:
	pr_debug("binder_mmap: %d %lx-%lx %s failed %d\n",
