...
struct cpufreq_interactive_cpuinfo {
	...
	unsigned int last_updated_cpu_load;
#endif
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
#define UPDATE_CPU_LOAD_MIN_INTERVAL 4000 /* 4000us */
unsigned int get_cpu_load(int cpu, u64 now)
{
	unsigned int delta_time_from_window_start, delta_time_from_last_update;
	u64 cputime_speedadj;
	unsigned long flags;
	unsigned int loadadjfreq;
	...
	/* For io_is_busy case, this feature must be disabled.
	   Io_is_busy request higher cpufreq and tasks should not be spread.
	   Always make cpu_overloaded=false in this case. */
	if (tunables->io_is_busy) {
		cpu_load = 0;
		goto exit;
	}

	delta_time_from_window_start = (unsigned int)(now - pcpu->cputime_speedadj_timestamp);
	delta_time_from_last_update = (unsigned int)(now - pcpu->time_in_idle_timestamp);

	/* Do not consider any load in the beginning part */
	if (delta_time_from_window_start < (tunables->timer_rate >> 1)) {
		cpu_load = 0;
		goto exit;
	}

	/* Use last load if not too long ago */
	if (delta_time_from_last_update < UPDATE_CPU_LOAD_MIN_INTERVAL) {
		cpu_load = pcpu->last_updated_cpu_load;
		goto exit;
	}
	...
	/* Calculate the cpu_load */
	cputime_speedadj = pcpu->cputime_speedadj;
	do_div(cputime_speedadj, delta_time_from_window_start);
	loadadjfreq = (unsigned int)cputime_speedadj * 100;
	cpu_load = loadadjfreq / pcpu->policy->cur;
	pcpu->last_updated_cpu_load = cpu_load;
