...
#define LOW_INT_VOL_SLEEP_TIME 125
...
#define FACTORY_SN_LEN  (16)
#define DATE_LEN (5)
char date_buf[DATE_LEN+1]={0};
...
static int low_temp_soc_threshold = 15;
				...
				int *ret_pc_unusable)
{
    ...
    if (di->rbatt_ratio){
        ...
        if (LOW_TEMP_OPT_OPEN == low_temp_opt_flag){
            if ((ratio > 0) && (ratio < 100) && (batt_temp_degc < low_temp_soc_threshold))
                ratio = ratio_min;
            if (1 != di->soc_limit_flag) {
                if (batt_temp_degc < low_temp_soc_threshold)
...
static unsigned int recalc_chargecycles(struct smartstar_coul_device *di)
{
 ...
 static int __init early_parse_factory_date_cmdline(char *p)
{
    char buf[FACTORY_SN_LEN + 1] = {0};
    int len;
    if (NULL == p)
    {
        hwlog_err("NULL point in [%s]\n", __func__);
    ...
    len = strlen(p);
    if (len != FACTORY_SN_LEN)
    {
        hwlog_err("androidboot.serialno length out of range\n");
        return -1;
    }
    memcpy(buf, p, len+1);
    memcpy(date_buf,buf+5,DATE_LEN);
    return 0;
}
early_param("androidboot.serialno", early_parse_factory_date_cmdline);
...
void update_bad_battery_flag(void)
{
	struct smartstar_coul_device *di = g_smartstar_coul_dev;
	struct ss_coul_nv_info *pinfo = &di->nv_info ;
	static int last_bad_battery_flag = -1;
	char buff[DSM_BUFF_SIZE_MAX] = {0};
	if ( 1 == di->chg_rbat_over_standard ) {
		di->bad_battery_flag = 1;
	} else {
		di->bad_battery_flag = 0;
	}

	hwlog_info("[%s]:chargecycles %d,chg_rbat_over_standard %d,batt_brand %s,date: %s\n",
		__func__,di->batt_chargecycles ,di->chg_rbat_over_standard,di->batt_data->batt_brand,date_buf);

	if(last_bad_battery_flag == di->bad_battery_flag)
		return;

	last_bad_battery_flag = di->bad_battery_flag;
	if (di->bad_battery_flag) {
		my_nv_info.bad_battery_flag =(short) BAD_BATTERY_NV_FLAG;
		pinfo->bad_battery_flag = (short) BAD_BATTERY_NV_FLAG;
		snprintf(buff, (size_t)DSM_BUFF_SIZE_MAX, "[BAD BATTERY] cur_temp:%d, chargecycles:%d, batt_brand %s,date: %s\n",
			di->batt_temp,di->batt_chargecycles,di->batt_data->batt_brand,date_buf);
		coul_dsm_report_ocv_cali_info(di, ERROR_BATT_NOT_EXIST, buff);
	} else {
		my_nv_info.bad_battery_flag = (short) GOOD_BATTERY_NV_FLAG;
		pinfo->bad_battery_flag = (short)  GOOD_BATTERY_NV_FLAG;
	}
	hwlog_info("[%s]:bad_battery_flag %d\n",__func__,di->bad_battery_flag);
	di->is_nv_need_save   = 1;
}

int coul_get_bad_battery_flag(void)
{
	struct smartstar_coul_device *di = g_smartstar_coul_dev;
	struct ss_coul_nv_info *pinfo = &di->nv_info ;
	unsigned short  chrg_bad_battery_flag = 0;
	static bool first_get_flag = true;
	if (!di->rbat_check_enable){
		return 0;
	}
	chrg_bad_battery_flag = my_nv_info.bad_battery_flag;
	if (first_get_flag) {
		if(BAD_BATTERY_NV_FLAG == chrg_bad_battery_flag) {
			my_nv_info.bad_battery_flag = (short) BAD_BATTERY_NV_FLAG;
			pinfo->bad_battery_flag = (short) BAD_BATTERY_NV_FLAG;
			di->bad_battery_flag = 1;
		} else {
		...
		hwlog_info("[%s]:chrg_bad_battery_flag_nv: 0x%x, bad_battery_flag %d, first_get_flag = %d\n",__func__,chrg_bad_battery_flag,di->bad_battery_flag,first_get_flag);
		first_get_flag = false;
	}
	hwlog_info("[%s]:bad_battery_flag %d\n",__func__,di->bad_battery_flag);
	return di->bad_battery_flag ;
}

void coul_save_chrg_rbat_over_standard(int  chrg_rbat_over_standard)
{
	struct smartstar_coul_device *di = g_smartstar_coul_dev;

	di->chg_rbat_over_standard = chrg_rbat_over_standard;

	hwlog_info("[%s]:chrg_rbat_over_standard %d\n", __func__,di->chg_rbat_over_standard);
	update_bad_battery_flag();
}
		...
		int batt_temp, int rbatt_tbl, int fcc_uah)
{
    ...
    if ((2 == di->soc_limit_flag) && (LOW_TEMP_OPT_OPEN == low_temp_opt_flag) && (batt_temp_degc < low_temp_soc_threshold)){
...
static void battery_plug_in(struct smartstar_coul_device *di)
{
    ...
    di->bad_battery_flag = 0;
    my_nv_info.bad_battery_flag = (short) GOOD_BATTERY_NV_FLAG;
    di->nv_info.bad_battery_flag = (short) GOOD_BATTERY_NV_FLAG;
    hwlog_info("new battery plug in, reset bad_battery_flag ,value = %d!\n",di->bad_battery_flag);
 ...
 static void coul_low_vol_int_handle(struct smartstar_coul_device *di)
{
    int ibat_ua = 0, vbat_uv = 0;
    int delta_soc = 0;
    int count = LOW_INT_VOL_COUNT, count_max = 0;
    int high_count = 0, high_temp = 0, low_count = 0;
    char buff[DSM_BUFF_SIZE_MAX] = {0};
    ...
    count_max = di->low_vol_filter_cnt * 1000 / LOW_INT_VOL_SLEEP_TIME;
    count = count_max;
    if (di->v_low_int_value == LOW_INT_STATE_SLEEP){
		if ((vbat_uv - LOW_INT_VOL_OFFSET) > (int)di->v_cutoff_sleep){
			...
			while(count--)
			{
				...
				coul_get_battery_voltage_and_current(di, &ibat_ua, &vbat_uv);
				hwlog_err("delay %d ms get vbat_uv is %duv,ibat_ua is %d uA!\n", LOW_INT_VOL_SLEEP_TIME, vbat_uv,ibat_ua);
				di->coul_dev_ops->show_key_reg();
				if ((vbat_uv/1000 - LOW_INT_VOL_OFFSET) > (int)di->v_cutoff_sleep){
					hwlog_err("fifo0 is false,it's got in 32k clk period!\n");
					high_count += 1;
					high_temp += 1;
					if (high_temp >= LOW_INT_VOL_HIGH_THRESHOLD) {
						hwlog_err("%d times the sampling voltage is higher than %duv", LOW_INT_VOL_HIGH_THRESHOLD, di->v_cutoff);
						return;
					}
				} else {
					high_temp = 0;
			...
			if (low_count >= count_max * LOW_INT_VOL_PERCENT / 100) {
				delta_soc = di->batt_soc - 2;
				di->batt_soc = 1;
			} else {
				hwlog_err("LOW_INT_STATE_SLEEP: low_count = %d, high_count=%d\n", low_count, high_count);
				return;
			}
		}
    }
		...
		if ((vbat_uv - LOW_INT_VOL_OFFSET) > di->v_cutoff){
			...
			if(low_count >= count_max * LOW_INT_VOL_PERCENT / 100) {
				...
				hwlog_err("LOW_INT_STATE_RUNNING: low_count = %d, high_count=%d\n", low_count, high_count);
				return;
			}
		}
    }
...
static void __fatal_isc_chg_prot(struct iscd_info *iscd, unsigned int * capacity)
{
    int ret;
                                             ...
                                             unsigned long event, void *data)
{
    ...
    if(event == ISC_LIMIT_START_CHARGING_STAGE) {
        ...
        __fatal_isc_chg_prot(iscd, (unsigned int *)data);
...
static void __fatal_isc_direct_chg_prot(struct iscd_info *iscd, unsigned int * capacity)
{
    int ret;
                                                       ...
                                                       unsigned long event, void *data)
{
    ...
    if(event == ISC_LIMIT_START_CHARGING_STAGE) {
        ...
        __fatal_isc_direct_chg_prot(iscd, (unsigned int *)data);
    }else if(event == ISC_LIMIT_UNDER_MONITOR_STAGE) {
        __fatal_isc_direct_chg_prot(iscd, (unsigned int *)data);
    }
    return NOTIFY_OK;
...
static void __fatal_isc_uevent(struct work_struct *work)
{
    ...
    hwlog_info("iscd uevent send in %s\n", __func__);
    iscd->isc_prompt = 1;
...
static void isc_splash2_file_sync(struct iscd_info *iscd)
{
    ...
    if(iscd->isc_splash2_ready){
        ...
        write_size = kernel_write(fd, (const void *)&iscd->fatal_isc_hist, sizeof(isc_history), (loff_t)0);
        if(write_size != (ssize_t)sizeof(iscd->fatal_isc_hist)) {
...
static void fatal_isc_dmd_wkfunc(struct work_struct *work)
{
    ...
    fatal_isc_dmd *reporter = container_of(dw, fatal_isc_dmd, work);
                                        ...
                                        unsigned long event, void *data)
{
    ...
    if(iscd->isc_status && iscd->fatal_isc_hist.dmd_report && event == ISC_LIMIT_BOOT_STAGE) {
        ...
        if(!iscd->dmd_reporter.buff) {
            iscd->dmd_reporter.buff = (char *)kzalloc((unsigned long)(unsigned)((j+1)*FATAL_ISC_DMD_LINE_SIZE+1), GFP_KERNEL);
            if(!iscd->dmd_reporter.buff) {
...
{
    unsigned int capacity = get_bci_soc();
...
{
    struct delayed_work *dw = container_of(work, struct delayed_work, work);
    struct iscd_info *iscd = container_of(dw, struct iscd_info, isc_limit_work);
    ...
    for(j = iscd->fatal_isc_trigger.valid_num; j > 0; j--) {
        for( i = last; i >= 0; i--) {
            ...
            if(time_limit) {
                ...
                if( delt_day > (int)iscd->fatal_isc_trigger.deadline || delt_day < 0) {
                    break;
                }
            }
        }
...
static void fatal_isc_judgement(struct iscd_info *iscd, unsigned int type)
{
    if(!iscd) {
        hwlog_err("Null input pointer iscd found in %s\n", __func__);
        return;
                            ...
                            int (*valid)(struct iscd_info *iscd, struct rtc_time *tm))
{
    ...
    if(valid(di->iscd, &tm)) {
        if(di->iscd->fatal_isc_hist.valid_num < MAX_FATAL_ISC_NUM) {
            ...
            di->iscd->fatal_isc_hist.year[di->iscd->fatal_isc_hist.valid_num] = (unsigned short)tm.tm_year;
            di->iscd->fatal_isc_hist.yday[di->iscd->fatal_isc_hist.valid_num] = (unsigned short)tm.tm_yday;
            ...
            for(i = 0; i < MAX_FATAL_ISC_NUM-1; i++) {
                di->iscd->fatal_isc_hist.isc[i] = di->iscd->fatal_isc_hist.isc[i+1L];
                di->iscd->fatal_isc_hist.rm[i] = di->iscd->fatal_isc_hist.rm[i+1L];
                di->iscd->fatal_isc_hist.fcc[i] = di->iscd->fatal_isc_hist.fcc[i+1L];
                di->iscd->fatal_isc_hist.qmax[i] = di->iscd->fatal_isc_hist.qmax[i+1L];
                di->iscd->fatal_isc_hist.charge_cycles[i] = di->iscd->fatal_isc_hist.charge_cycles[i+1L];
                di->iscd->fatal_isc_hist.year[i] = di->iscd->fatal_isc_hist.year[i+1L];
                di->iscd->fatal_isc_hist.yday[i] = di->iscd->fatal_isc_hist.yday[i+1L];
            }
            ...
            di->iscd->fatal_isc_hist.year[i] = (unsigned short)tm.tm_year;
            di->iscd->fatal_isc_hist.yday[i] = (unsigned short)tm.tm_yday;
        } else {
            ...
            di->iscd->fatal_isc_hist.year[0] = (unsigned short)tm.tm_year;
            di->iscd->fatal_isc_hist.yday[0] = (unsigned short)tm.tm_yday;
        }
...
enum coul_sysfs_type{
    ...
    COUL_SYSFS_CALI_ADC,
#ifdef ISC_TEST
    COUL_SYSFS_ISC_STATUS,
    COUL_SYSFS_FATAL_ISC,
    COUL_SYSFS_ISC_PROMPT,
    COUL_SYSFS_ISC_DMD_ONLY,
    COUL_SYSFS_ISC_TRIGGER_TYPE,
    COUL_SYSFS_ISC_CHARGE_LIMIT_SOC,
    COUL_SYSFS_ISC_RECHARGE_SOC,
    COUL_SYSFS_ISC_VALID_CYCLES,
    COUL_SYSFS_NEED_MONITOR,
#endif
};
...
#ifdef ISC_TEST
    COUL_SYSFS_FIELD_RW(isc_status,             ISC_STATUS),
    COUL_SYSFS_FIELD_RW(fatal_isc,              FATAL_ISC),
    COUL_SYSFS_FIELD_RW(isc_prompt,             ISC_PROMPT),
    COUL_SYSFS_FIELD_RW(isc_dmd_only,           ISC_DMD_ONLY),
    COUL_SYSFS_FIELD_RW(isc_trigger_type,       ISC_TRIGGER_TYPE),
    COUL_SYSFS_FIELD_RW(isc_charge_limit_soc,   ISC_CHARGE_LIMIT_SOC),
    COUL_SYSFS_FIELD_RW(isc_recharge_soc,       ISC_RECHARGE_SOC),
    COUL_SYSFS_FIELD_RW(isc_valid_cycles,       ISC_VALID_CYCLES),
    COUL_SYSFS_FIELD_RW(need_monitor,           NEED_MONITOR),
#endif
};
/*lint +e665*/
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case COUL_SYSFS_CALI_ADC:
        return snprintf(buf, PAGE_SIZE, "%d\n", 0);
#ifdef ISC_TEST
    case COUL_SYSFS_ISC_STATUS:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->isc_status);
    case COUL_SYSFS_FATAL_ISC:
        val = snprintf(buf, PAGE_SIZE, "status:%02x trigger:%02x valid num:%02x  dmd:%02x  version:%08x\n",
                       di->iscd->fatal_isc_hist.isc_status, di->iscd->fatal_isc_hist.trigger_type,
                       di->iscd->fatal_isc_hist.valid_num, di->iscd->fatal_isc_hist.dmd_report,
                       di->iscd->fatal_isc_hist.magic_num);
        ...
        for( i = 0; i < MAX_FATAL_ISC_NUM; i++) {
            val += snprintf(buf + val, PAGE_SIZE, "%11d%11d%11d%11d%11d%6d%6d\n",
                            di->iscd->fatal_isc_hist.isc[i], di->iscd->fatal_isc_hist.fcc[i],
                            di->iscd->fatal_isc_hist.rm[i], di->iscd->fatal_isc_hist.qmax[i],
                            di->iscd->fatal_isc_hist.charge_cycles[i], di->iscd->fatal_isc_hist.year[i],
                            di->iscd->fatal_isc_hist.yday[i]);
            if(val > (int)(PAGE_SIZE >> 1)) {
                break;
            }
        }
        return val;
    case COUL_SYSFS_ISC_PROMPT:
        val = snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->isc_prompt);
        di->iscd->isc_prompt = 0;
        return val;
    case COUL_SYSFS_ISC_DMD_ONLY:
        return snprintf(buf, PAGE_SIZE, "%d\n",
                        !(di->iscd->fatal_isc_action & (~BIT(UPLOAD_DMD_ACTION))));
    case COUL_SYSFS_ISC_TRIGGER_TYPE:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->fatal_isc_trigger_type);
    case COUL_SYSFS_ISC_CHARGE_LIMIT_SOC:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->fatal_isc_soc_limit[UPLIMIT]);
    case COUL_SYSFS_ISC_RECHARGE_SOC:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->fatal_isc_soc_limit[RECHARGE]);
    case COUL_SYSFS_ISC_VALID_CYCLES:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->isc_valid_cycles);
    case COUL_SYSFS_NEED_MONITOR:
        val = snprintf(buf, PAGE_SIZE, "%d\n", di->iscd->need_monitor);
        return val;
#endif
    default:
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    case COUL_SYSFS_FATAL_ISC:
        ...
        break;
    case COUL_SYSFS_ISC_PROMPT:
        __fatal_isc_uevent(&(di->iscd->fatal_isc_uevent_work));
        break;
    case COUL_SYSFS_ISC_DMD_ONLY:
        if ( kstrtol(buf, 10, &val) < 0 )
            return -EINVAL;
        val = !!val;
        di->iscd->fatal_isc_action = 0;
        if(di->iscd->fatal_isc_trigger_type != INVALID_ISC_JUDGEMENT) {
            if(val != FATAL_ISC_DMD_ONLY) {
                di->iscd->fatal_isc_action |= BIT(NORAML_CHARGING_ACTION);
                di->iscd->fatal_isc_action |= BIT(DIRECT_CHARGING_ACTION);
                di->iscd->fatal_isc_action |= BIT(UPLOAD_UEVENT_ACTION);
            }
            di->iscd->fatal_isc_action |= BIT(UPLOAD_DMD_ACTION);
        }
        set_fatal_isc_action(di->iscd);
        break;
    case COUL_SYSFS_ISC_TRIGGER_TYPE:
        if ( kstrtol(buf, 10, &val) < 0 || di->iscd->isc_status)
            return -EINVAL;
        di->iscd->fatal_isc_trigger_type = (unsigned int)val;
        break;
    case COUL_SYSFS_ISC_CHARGE_LIMIT_SOC:
        if ((kstrtol(buf, 10, &val) < 0) || (val < 0) || (val > 100))
            return -EINVAL;
        di->iscd->fatal_isc_soc_limit[UPLIMIT] = ((unsigned int)val <= di->iscd->fatal_isc_soc_limit[RECHARGE]) ?
                                          (di->iscd->fatal_isc_soc_limit[RECHARGE] + 1) : (unsigned int)val;
        break;
    case COUL_SYSFS_ISC_RECHARGE_SOC:
        if ((kstrtol(buf, 10, &val) < 0) || (val < 0) || (val > 100))
            return -EINVAL;
        di->iscd->fatal_isc_soc_limit[RECHARGE] = ((unsigned int)val >= di->iscd->fatal_isc_soc_limit[UPLIMIT]) ?
                                           (di->iscd->fatal_isc_soc_limit[UPLIMIT] - 1) : (unsigned int)val;
        break;
    case COUL_SYSFS_ISC_VALID_CYCLES:
        if (kstrtol(buf, 10, &val) < 0)
            return -EINVAL;
        di->iscd->isc_valid_cycles = (int)val;
        break;
    case COUL_SYSFS_NEED_MONITOR:
        if (kstrtol(buf, 10, &val) < 0)
            return -EINVAL;
        di->iscd->need_monitor = (int)val;
        break;
#endif
    default:
        hwlog_err("(%s)NODE ERR!!HAVE NO THIS NODE:(%d)\n",__func__,info->name);
...
static void coul_core_get_iscd_info(struct device_node* np, struct smartstar_coul_device* di)
{
    ...
    di->iscd->isc_valid_cycles = ISCD_CHARGE_CYCLE_MIN;
}
...
static void coul_core_get_dts(struct smartstar_coul_device *di)
{
    ...
    unsigned int rbat_check_enable = 0;
    unsigned int low_temp_soc_speedup = 0;
    unsigned int low_temp_soc_thr = 15;
    ...
    if (of_property_read_u32(np, "rbat_check_enable",&rbat_check_enable)){
        hwlog_err("dts:can not get rbat_check_enable,use default : %d!\n",rbat_check_enable);
    }
    di->rbat_check_enable = rbat_check_enable;
    hwlog_info("dts:get rbat_check_enable = %d! \n",rbat_check_enable);

    if (of_property_read_u32(np, "low_temp_soc_speedup",&low_temp_soc_speedup)){
        hwlog_err("dts:can not get low_temp_soc_speedup,use default : %d!\n",low_temp_soc_speedup);
    }
    low_temp_opt_flag = low_temp_soc_speedup;
    hwlog_info("dts:get low_temp_soc_speedup = %d! \n",low_temp_soc_speedup);

    if (of_property_read_u32(np, "low_temp_soc_threshold",&low_temp_soc_thr)){
        hwlog_err("dts:can not get low_temp_soc_thr,use default : %d!\n",low_temp_soc_thr);
    }
    low_temp_soc_threshold = low_temp_soc_thr;
    hwlog_info("dts:get low_temp_soc_threshold = %d! \n",low_temp_soc_threshold);
...
int get_isc_reset_flag(void)
{
    if(!g_smartstar_coul_dev) {
        hwlog_err("g_smartstar_coul_dev is null found in %s.\n", __func__);
        return 0;
    }
    return g_smartstar_coul_dev->isc_reset_flag;
}
...
static void isc_hist_info_init(struct work_struct *work)
{
    ...
    mm_segment_t old_fs;
    struct delayed_work *dw = container_of(work, struct delayed_work, work);
    struct iscd_info *iscd = container_of(dw, struct iscd_info, isc_splash2_work);
    ...
    read_size = kernel_read(fd, (loff_t)0, buff, MOUNTS_INFO_FILE_MAX_SIZE);
    if(read_size < 0) {
    ...
    if (file_des < 0) {
        hwlog_info("Access directory %s failed(%ld) in %s.\n",
                   ISC_DATA_DIRECTORY, file_des, __func__);
        file_des = sys_mkdir(ISC_DATA_DIRECTORY, 0770);
        if (file_des < 0) {
            hwlog_err("Create directory %s for recording fatal isc failed(%ld) in %s\n",
                      ISC_DATA_DIRECTORY, file_des, __func__);
            set_fs(old_fs);
    ...
    flags = O_RDWR | O_CREAT | (get_isc_reset_flag() ? O_TRUNC : 0);
    fd = filp_open(ISC_DATA_FILE, flags, 0660);
    if (IS_ERR(fd)) {
        hwlog_err("Open and create %s failed in %s.\n", ISC_DATA_FILE, __func__);
        goto isc_init_buff_free;
    }
    read_size = kernel_read(fd, (loff_t)0, buff, sizeof(isc_history)+1);
    if(read_size == (int)sizeof(isc_history)) {
        hwlog_info("fatal isc datum file size was correct.\n");
        memcpy(&iscd->fatal_isc_hist, buff, sizeof(isc_history));
...
static ssize_t isc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "%u", iscd->isc_status);
}
static ssize_t isc_limit_support_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "%u", iscd->fatal_isc_trigger_type);
}
...
static void fatal_isc_init(struct device_node* np, struct iscd_info *iscd)
{
    ...
    if(!battery) {
        ...
        ret = device_create_file(battery, &dev_attr_isc);
        if(ret) {
            hwlog_err("create file isc under battery failed in %s.\n", __func__);
        }
        ret = device_create_file(battery, &dev_attr_isc_limit_support);
        if(ret) {
            hwlog_err("create file isc_limit_support under battery failed in %s.\n", __func__);
        }
    }
...
static int  hisi_coul_probe(struct platform_device *pdev)
{
    ...
    if(di->coul_dev_ops->is_battery_moved()){
        ...
        my_nv_info.bad_battery_flag = (short) GOOD_BATTERY_NV_FLAG;
        di->nv_info.bad_battery_flag = (short) GOOD_BATTERY_NV_FLAG;
        hwlog_info("battery changed, reset bad_battery_flag ,value = %d!\n",di->bad_battery_flag);
    } else {
    ...
    di->chg_rbat_over_standard = coul_get_bad_battery_flag();
    hwlog_info("chg_rbat_over_standard %d\n",di->chg_rbat_over_standard);
    /*low temp soc speedup works only when bad battery flag is ture*/
    if (di->bad_battery_flag && low_temp_opt_flag){
        low_temp_opt_flag = LOW_TEMP_OPT_OPEN;
    } else {
        low_temp_opt_flag = LOW_TEMP_OPT_CLOSE;
    }
    hwlog_info("low_temp_opt_flag  %d\n",low_temp_opt_flag);
...
coul_no_battery:
    ...
    coul_ops->save_chrg_rbat_over_standard = coul_save_chrg_rbat_over_standard;
    coul_ops->get_bad_battery_flag        = coul_get_bad_battery_flag;
	...
	g_pdev = pdev;
    /* fatal isc init */
    fatal_isc_init(di->dev->of_node, di->iscd);
