...
void hisi_save_chrg_rbat_over_standard(int chrg_rbat_over_standard)
{
	LOCAL_HISI_COUL_OPS();

	if (ops && ops->save_chrg_rbat_over_standard)
	{
		ops->save_chrg_rbat_over_standard(chrg_rbat_over_standard);
	}
}

int hisi_get_bad_battery_flag(void)
{
	/*declare the local variable of struct hisi_coul_ops */
	LOCAL_HISI_COUL_OPS();

	/*execute the operation of coul module */
	HISI_EXEC_COUL_OP(get_bad_battery_flag);
