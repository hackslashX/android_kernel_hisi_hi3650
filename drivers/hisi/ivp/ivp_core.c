...
#define LISTENTRY_SIZE          (0x00100000)
...
inline void ivp_reg_write(unsigned int off, u32 val)
{
    ...
    ivp_dbg("REG WRITE:0x%pK = 0x%08x", reg, val);
    writel(val, reg);
}
...
inline u32 ivp_reg_read(unsigned int off)
{
    char __iomem * reg = ivp_dev.io_res.cfg_base_addr + off;
    u32 val = readl(reg);
    ivp_dbg("REG READ:0x%pK = 0x%08x", reg, val);
    return val;
}
...
{
    ivp_reg_write(IVP_REG_OFF_STATVECTORSEL, (unsigned)mode & 0x01);
}

static inline void ivp_hw_clockgate(struct ivp_device *devp, int state)
{
    ivp_reg_write(IVP_REG_OFF_DSPCORE_GATE, (unsigned)state & 0x01);
}
...
{
    int val;
    ivp_reg_write(IVP_REG_OFF_RUNSTALL, (unsigned)mode & 0x01);
...
static inline void ivp_hw_set_mem_sleepmode(int off, int mode, int mask)
{
    ...
    unsigned int new_mode = (old_mode & (~(unsigned)mask)) | mode ;
...
static int ivp_get_memory_section(struct ivp_device *devp)
{
    int i = 0;
    ...
    devp->vaddr_memory = NULL;
    if ((devp->vaddr_memory = dma_alloc_coherent(devp->device.this_device,
                    LISTENTRY_SIZE, &dma_addr, GFP_KERNEL)) == NULL) {
        ivp_err("[%s] ivp_get_vaddr.0x%pK\n", __func__, devp->vaddr_memory);
        return -EINVAL;
    }

    for(i = 3; i < devp->sect_count; i++) {
        if(i == 3) {
            devp->sects[i].acpu_addr = dma_addr >> 4;
        }
        else {
            devp->sects[i].acpu_addr = ((devp->sects[i-1].acpu_addr << 4)
                     + devp->sects[i-1].len) >> 4;
            devp->sects[i].ivp_addr = devp->sects[i-1].ivp_addr
                     + devp->sects[i-1].len ;
        }
        devp->sects[i].len = SIZE_2MB;
        ivp_dbg("________ivp sections 0x%pK\n", devp->sects[i].acpu_addr);
    }
    return 0;
}

static int ivp_free_memory_section(struct ivp_device *devp)
{
    dma_addr_t dma_addr = 0;
    dma_addr = devp->sects[3].acpu_addr << 4;

    dma_free_coherent(devp->device.this_device, LISTENTRY_SIZE,
            devp->vaddr_memory, dma_addr);
    devp->vaddr_memory = NULL;
	return 0;
}
...
static int ivp_dev_poweron(struct ivp_device *devp)
{
    ...
    mutex_unlock(&ivp_wake_lock_mutex);
    ...
    ivp_info("set core success to: %ld", clk_get_rate(devp->clk));
...
static long ivp_ioctl(struct file *fd, unsigned int cmd, unsigned long args)
{
    ...
    case IVP_IOCTL_SECTINFO: {
        ...
        ivp_dbg("section %d:physical address=0x%016lx;length=0x%08x.",
...
static int ivp_mmap(struct file *fd, struct vm_area_struct *vma)
{
    ...
    ivp_dbg("vm_start=0x%016lx:vm_end=0x%016lx:vm_offset=0x%016lx:mm_off:%#016lx",
    ...
    for (; i < ivp_dev.sect_count; i++) {
        ivp_dbg("Target addr=0x%016lx:length=0x%08x;source addr=0x%016lx:length=0x%08x.",
              phy_addr, size,
...
static inline int ivp_setup_one_onchipmem_section(struct ivp_sect_info *sect, struct device_node *np)
{
     ...
     sect->len       = settings[2];
     ivp_dbg("name = %s : acpu-addr = 0x%08lx : len = 0x%08x : ivp-addr = 0x%08x.",
...
static int ivp_init_reg_res(struct platform_device *pdev, struct ivp_device *ivp_devp)
{
    ...
    ivp_dbg("get cfgbase:%#llx, map:0x%pK", mem_res->start, ivp_devp->io_res.cfg_base_addr);
    ...
    ivp_dbg("get pctrlbase:%#llx, map:0x%pK", mem_res->start, ivp_devp->io_res.pctrl_base_addr);
    ...
    ivp_dbg("get pericrgbase:%#llx, map:0x%pK", mem_res->start, ivp_devp->io_res.pericrg_base_addr);
